package vn.vndgroup.ekycnavite
...
import com.vnd.ekycnavite.CameraPreviewActivity
import vn.vndgroup.ekyc.objectdetection.IdCardType
import vn.vndgroup.ekycnavite.global.Constants
import vn.vndgroup.ekycnavite.databinding.ActivityMainBinding
import java.io.File

class MainActivity : AppCompatActivity() {
    private lateinit var binding: ActivityMainBinding
    private val takePhotoContract =
        registerForActivityResult(CameraPreviewActivity.Contract()) { result ->
    ...
    private fun showImage(url: String?) {
        if (!url.isNullOrEmpty()) {
            startActivity(
                Intent(this, ImagePreviewActivity::class.java).putExtra(
                    "data",
                    url
                )
            )
